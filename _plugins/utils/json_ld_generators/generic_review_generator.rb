  ...
  def self.generate_hash(document, site)
    ...
    if item_name.nil? || item_name.to_s.strip.empty?
      ...
      Jekyll.logger.error 'JSON-LD (GenericReviewGen):',
                          "Called for '#{doc_identifier}' but 'review.item_name' is missing or empty. This should have been caught by the injector."
    ...
    item_reviewed['url'] = UrlUtils.absolute_url(item_url_fm, site) if item_url_fm && !item_url_fm.strip.empty?
