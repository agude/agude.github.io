...
module Jekyll
  ...
  module SEO
    module Generators
      # Utility module for generating JSON-LD BlogPosting schema.
      # Creates structured data for blog post pages.
      module BlogPostingLdGenerator
        ...
        private_class_method def self.add_image(data, document, site)
          image_entity = Jekyll::SEO::JsonLdUtils.build_image_object_entity(document.data['image'], site)
          data['image'] = image_entity if image_entity
        end

        private_class_method def self.add_url_and_main_entity(data, document, site)
          doc_url_abs = Jekyll::Infrastructure::UrlUtils.absolute_url(document.url, site)
