...
module BookCardUtils
  ...
  def self.render(book_object, context, display_title_override: nil, subtitle: nil)
    Renderer.new(book_object, context, display_title_override, subtitle).render
  end
  ...
  class Renderer
    ...
    def render
      @base = CardDataExtractorUtils.extract_base_data(
        @book_obj, @context, default_title: DEFAULT_TITLE_FOR_BOOK_CARD, log_tag_type: 'BOOK_CARD_UTIL'
      )
