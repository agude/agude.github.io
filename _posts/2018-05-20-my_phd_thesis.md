...
part of that endeavor, I spent years traveling to and from [CERN][cern] where I
...
any messy QCD interactions to confound the data. More specifically, I looked at
...
space with a lot of energy which is dissipated by creating new particles.
...
experiments: [ATLAS][atlas]; [ALICE][alice]; [LHCb][lhcb]; and CMS, the
[Compact Muon Solenoid][cms], which was my experiment.
...
events that CMS recorded with those characteristics and compare the data to our
prediction.
...
about the low energy regime of QCD, but it also helps to constrain the mass of
...
example, since I cared about double electron events, we tuned our MC on events
with one electron and one [muon][muon] (which is very much like an electron,
but heavier).
...
by using MC events and putting them through the analysis pipeline. This lets us
check what the analysis would have measured, while having information about the
true underlying event.
...
CMS, and the colored points are the number of events predicted by various Monte
Carlo generator programs that use different approximations of the Standard
Model. The bottom plot is the ratio of MC events over observed events. If the
...
points (from the [MadGraph][madgraph] generator) do the best job, but even they
miss by up to 5%.
