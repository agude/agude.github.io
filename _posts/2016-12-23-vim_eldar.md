...
![The Eldar color scheme logo]({{ site.url }}/files/vim_eldar/eldar_logo.svg)
...
I decided to create my own color scheme that would always look the way I
wanted. The fact that designing the scheme served double duty as [structured
procrastination][sp] to avoid [my thesis][thesis] (also written in Vim) was
merely a bonus!
...
![An example Python file using the Eldar color scheme]({{ site.url
}}/files/vim_eldar/eldar_python.png)
...
![Eldar vs elflord gif]({{ site.url }}/files/vim_eldar/eldar_elflord_cpp.gif)
