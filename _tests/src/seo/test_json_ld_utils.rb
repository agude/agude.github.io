...
# Tests for Jekyll::SEO::JsonLdUtils module.
#
...
class TestJsonLdUtils < Minitest::Test
  ...
  def test_build_site_person_entity_with_name
    ...
    assert_equal expected, Jekyll::SEO::JsonLdUtils.build_site_person_entity(@mock_site_full)
  end
  ...
  def test_build_site_person_entity_no_author_name
    assert_nil Jekyll::SEO::JsonLdUtils.build_site_person_entity(@mock_site_no_author)
  end

  def test_build_site_person_entity_empty_author_name
    site_empty_author = create_site({ 'author' => { 'name' => '  ' } })
    assert_nil Jekyll::SEO::JsonLdUtils.build_site_person_entity(site_empty_author)
  end
  ...
  def test_build_document_person_entity_with_name
    ...
    assert_equal expected, Jekyll::SEO::JsonLdUtils.build_document_person_entity('Book Author Name')
  end

  def test_build_document_person_entity_empty_name
    assert_nil Jekyll::SEO::JsonLdUtils.build_document_person_entity('')
    assert_nil Jekyll::SEO::JsonLdUtils.build_document_person_entity('   ')
  end

  def test_build_document_person_entity_nil_name
    assert_nil Jekyll::SEO::JsonLdUtils.build_document_person_entity(nil)
  end
  ...
  def test_build_image_object_entity_empty_path
    assert_nil Jekyll::SEO::JsonLdUtils.build_image_object_entity('', @mock_site_full)
    assert_nil Jekyll::SEO::JsonLdUtils.build_image_object_entity('  ', @mock_site_full)
  end

  def test_build_image_object_entity_nil_path
    assert_nil Jekyll::SEO::JsonLdUtils.build_image_object_entity(nil, @mock_site_full)
  end
  ...
  def test_build_rating_entity_valid
    ...
    assert_equal expected, Jekyll::SEO::JsonLdUtils.build_rating_entity(4)
    assert_equal expected, Jekyll::SEO::JsonLdUtils.build_rating_entity('4')
  end

  def test_build_rating_entity_custom_best_worst
    expected = { '@type' => 'Rating', 'ratingValue' => '3', 'bestRating' => '10', 'worstRating' => '0' }
    assert_equal expected, Jekyll::SEO::JsonLdUtils.build_rating_entity(3, best_rating: '10', worst_rating: '0')
  end

  def test_build_rating_entity_invalid_or_zero
    assert_nil Jekyll::SEO::JsonLdUtils.build_rating_entity(0)
    assert_nil Jekyll::SEO::JsonLdUtils.build_rating_entity(-1)
    assert_nil Jekyll::SEO::JsonLdUtils.build_rating_entity('abc')
    assert_nil Jekyll::SEO::JsonLdUtils.build_rating_entity(nil)
  end
  ...
  def test_build_book_series_entity_with_name_and_position
    ...
    assert_equal expected, Jekyll::SEO::JsonLdUtils.build_book_series_entity(' The Expanse ', '3')
  end

  def test_build_book_series_entity_with_name_only
    expected = { '@type' => 'BookSeries', 'name' => 'Dune' }
    assert_equal expected, Jekyll::SEO::JsonLdUtils.build_book_series_entity('Dune')
    assert_equal expected, Jekyll::SEO::JsonLdUtils.build_book_series_entity('Dune', 0) # Position 0 is invalid
    assert_equal expected, Jekyll::SEO::JsonLdUtils.build_book_series_entity('Dune', 'abc') # Invalid position
  end

  def test_build_book_series_entity_empty_or_nil_name
    assert_nil Jekyll::SEO::JsonLdUtils.build_book_series_entity('', '1')
    assert_nil Jekyll::SEO::JsonLdUtils.build_book_series_entity('   ', '1')
  ...
  def test_cleanup_data_hash_removes_nil_and_empty
    ...
    assert_equal expected, Jekyll::SEO::JsonLdUtils.cleanup_data_hash!(data)
  end
  ...
  # --- Tests for clean_array_field ---
  def test_clean_array_field_valid_array
    doc_data = { 'awards' => ['Hugo Award', 'Nebula Award'], 'url' => '/books/dune.html' }
    result = Jekyll::SEO::JsonLdUtils.clean_array_field(doc_data, 'awards', 'award', 'JSON-LD:')
    assert_equal({ 'award' => ['Hugo Award', 'Nebula Award'] }, result)
  end

  def test_clean_array_field_dirty_array
    doc_data = { 'tags' => [' foo ', '', nil, 'bar', '  '], 'url' => '/test' }
    result = Jekyll::SEO::JsonLdUtils.clean_array_field(doc_data, 'tags', 'keywords', 'JSON-LD:')
    assert_equal({ 'keywords' => %w[foo bar] }, result)
  end

  def test_clean_array_field_empty_array
    doc_data = { 'awards' => [], 'url' => '/test' }
    result = Jekyll::SEO::JsonLdUtils.clean_array_field(doc_data, 'awards', 'award', 'JSON-LD:')
    assert_equal({}, result)
  end

  def test_clean_array_field_nil_value
    doc_data = { 'awards' => nil, 'url' => '/test' }
    result = Jekyll::SEO::JsonLdUtils.clean_array_field(doc_data, 'awards', 'award', 'JSON-LD:')
    assert_equal({}, result)
  end

  def test_clean_array_field_missing_key
    doc_data = { 'url' => '/test' }
    result = Jekyll::SEO::JsonLdUtils.clean_array_field(doc_data, 'awards', 'award', 'JSON-LD:')
    assert_equal({}, result)
  end

  def test_clean_array_field_non_array_logs_warning
    doc_data = { 'awards' => 'Not an array', 'url' => '/books/test.html' }
    mock_logger = Minitest::Mock.new
    mock_logger.expect(:warn, nil) do |prefix, msg|
      prefix == 'JSON-LD:' &&
        msg.include?('awards') &&
        msg.include?('/books/test.html') &&
        msg.include?('not an Array')
    end
    result = nil
  ...
  def test_clean_array_field_non_array_uses_path_fallback
    doc_data = { 'awards' => 42, 'path' => '_books/fallback.md' }
    mock_logger = Minitest::Mock.new
    mock_logger.expect(:warn, nil) { |_p, m| m.include?('_books/fallback.md') }
    Jekyll.stub :logger, mock_logger do
      Jekyll::SEO::JsonLdUtils.clean_array_field(doc_data, 'awards', 'award', 'JSON-LD:')
    end
    mock_logger.verify
  end

  def test_clean_array_field_doc_id_overrides_data_url
    doc_data = { 'awards' => 'Not an array', 'url' => '/should-not-appear' }
    mock_logger = Minitest::Mock.new
    mock_logger.expect(:warn, nil) do |_prefix, msg|
      msg.include?('/explicit/id.html') && !msg.include?('/should-not-appear')
    end
    Jekyll.stub :logger, mock_logger do
