...
# Tests for Jekyll::SEO::Generators::GenericReviewGenerator class.
#
# Verifies that the generator correctly creates JSON-LD structured data for generic product reviews.
class TestGenericReviewGenerator < Minitest::Test
  def setup
    @site_config = {
  ...
  def test_generate_hash_returns_empty_if_item_name_is_missing_in_generator_guard
    ...
    Jekyll.stub :logger, mock_logger do
      actual_hash = Jekyll::SEO::Generators::GenericReviewGenerator.new(doc_missing_item_name, @site).generate
    end
