...
# Tests for Jekyll::Infrastructure::Links::LinkHelperUtils module.
#
...
class TestLinkHelperUtils < Minitest::Test
  ...
  def test_get_display_text_basic_input
    assert_equal 'Input Text', Jekyll::Infrastructure::Links::LinkHelperUtils._get_link_display_text(' Input Text ', nil, nil)
  end

  def test_get_display_text_with_override
    doc = create_doc({ 'title' => 'Canonical Title' })
    # The helper now strips the override text
    assert_equal 'Override Text', Jekyll::Infrastructure::Links::LinkHelperUtils._get_link_display_text('Input', ' Override Text ', doc)
  end

  def test_get_display_text_with_found_doc_no_override
    doc = create_doc({ 'title' => ' Canonical Title ' })
    assert_equal 'Canonical Title', Jekyll::Infrastructure::Links::LinkHelperUtils._get_link_display_text('Input', nil, doc)
  end
  ...
  def test_get_display_text_with_found_doc_empty_title
    ...
    assert_equal 'Input Text', Jekyll::Infrastructure::Links::LinkHelperUtils._get_link_display_text(' Input Text ', nil, doc)
  end
  ...
  def test_get_display_text_nil_input
    ...
    assert_equal '', Jekyll::Infrastructure::Links::LinkHelperUtils._get_link_display_text(nil, nil, nil)
    # Helper now strips override text
    assert_equal 'Override', Jekyll::Infrastructure::Links::LinkHelperUtils._get_link_display_text(nil, ' Override ', nil)
    assert_equal 'Canonical', Jekyll::Infrastructure::Links::LinkHelperUtils._get_link_display_text(nil, nil, doc)
    # Helper now strips override text
    assert_equal 'Override', Jekyll::Infrastructure::Links::LinkHelperUtils._get_link_display_text(nil, ' Override ', doc)
  end

  def test_get_display_text_empty_input
    doc = create_doc({ 'title' => 'Canonical' })
    assert_equal '', Jekyll::Infrastructure::Links::LinkHelperUtils._get_link_display_text('', nil, nil)
    # Helper now strips override text
    assert_equal 'Override', Jekyll::Infrastructure::Links::LinkHelperUtils._get_link_display_text('', ' Override ', nil)
    assert_equal 'Canonical', Jekyll::Infrastructure::Links::LinkHelperUtils._get_link_display_text('', nil, doc)
    # Helper now strips override text
    assert_equal 'Override', Jekyll::Infrastructure::Links::LinkHelperUtils._get_link_display_text('', ' Override ', doc)
  end
  ...
  def test_generate_link_to_anchor_on_same_page
    ...
    assert_equal expected, Jekyll::Infrastructure::Links::LinkHelperUtils._generate_link_html(ctx, target_url_with_fragment, inner_html)
  end
  ...
  def test_generate_link_to_same_page_no_anchor_is_suppressed
    ...
    assert_equal expected, Jekyll::Infrastructure::Links::LinkHelperUtils._generate_link_html(ctx, target_url_no_fragment, inner_html)
  end
  ...
  def test_generate_link_suppresses_links_between_canonical_and_archived
    ...
    output_from_archived = Jekyll::Infrastructure::Links::LinkHelperUtils._generate_link_html(ctx_from_archived, canonical_page.url, inner_html)
    ...
    output_from_canonical = Jekyll::Infrastructure::Links::LinkHelperUtils._generate_link_html(ctx_from_canonical, archived_page.url, inner_html)
  ...
  def test_generate_link_doc_not_found
    ...
    expected = inner_html # No link
    assert_equal expected, Jekyll::Infrastructure::Links::LinkHelperUtils._generate_link_html(ctx, nil, inner_html) # Pass nil URL
  end
  ...
  def test_generate_link_doc_found_no_url
    ...
    expected = inner_html # No link
    assert_equal expected, Jekyll::Infrastructure::Links::LinkHelperUtils._generate_link_html(ctx, found_doc.url, inner_html) # Pass nil URL
  end
  ...
  def test_integration_with_link_cache_generator_for_self_link_suppression
    ...
    output_from_archived = Jekyll::Infrastructure::Links::LinkHelperUtils._generate_link_html(ctx_from_archived, canonical_book.url, inner_html)
    assert_equal inner_html, output_from_archived, 'Link from archived to canonical should be suppressed'

    # Test from canonical page to archived page
    ctx_from_canonical = create_context({}, { site: site, page: canonical_book })
    output_from_canonical = Jekyll::Infrastructure::Links::LinkHelperUtils._generate_link_html(ctx_from_canonical, archived_book.url, inner_html)
    assert_equal inner_html, output_from_canonical, 'Link from canonical to archived should be suppressed'
  end
end
