...
# Tests for Jekyll::Infrastructure::TagArgumentUtils module.
#
...
class TestTagArgumentUtils < Minitest::Test
  ...
  def test_resolve_value_quoted_string
    ...
    assert_equal 'with space', Jekyll::Infrastructure::TagArgumentUtils.resolve_value("'with space'", ctx)
    assert_equal '', Jekyll::Infrastructure::TagArgumentUtils.resolve_value("''", ctx) # Empty quoted string
  end

  def test_resolve_value_variable_found_string
    ctx = create_context({ 'page' => { 'my_var' => 'found it' } })
    ctx['simple_var'] = 'top level'
    assert_equal 'found it', Jekyll::Infrastructure::TagArgumentUtils.resolve_value('page.my_var', ctx)
    assert_equal 'top level', Jekyll::Infrastructure::TagArgumentUtils.resolve_value('simple_var', ctx)
  end

  def test_resolve_value_variable_found_non_string
    ctx = create_context({ 'num_var' => 123, 'bool_var' => true })
    assert_equal 123, Jekyll::Infrastructure::TagArgumentUtils.resolve_value('num_var', ctx)
    assert_equal true, Jekyll::Infrastructure::TagArgumentUtils.resolve_value('bool_var', ctx)
  end

  def test_resolve_value_variable_holds_nil
    ctx = create_context({ 'nil_var' => nil })
    assert_nil Jekyll::Infrastructure::TagArgumentUtils.resolve_value('nil_var', ctx)
  end

  def test_resolve_value_variable_holds_false
    ctx = create_context({ 'false_var' => false })
    assert_equal false, Jekyll::Infrastructure::TagArgumentUtils.resolve_value('false_var', ctx)
  end

  def test_resolve_value_variable_not_found_returns_nil
    ctx = create_context
    assert_nil Jekyll::Infrastructure::TagArgumentUtils.resolve_value('missing_var', ctx)
    assert_nil Jekyll::Infrastructure::TagArgumentUtils.resolve_value('page.missing', ctx)
  end

  def test_resolve_value_unquoted_matches_existing_variable
    ctx = create_context({ 'my_literal' => 'variable value' })
    assert_equal 'variable value', Jekyll::Infrastructure::TagArgumentUtils.resolve_value('my_literal', ctx)
  end

  def test_resolve_value_unquoted_does_not_match_variable_returns_nil
    ctx = create_context
    assert_nil Jekyll::Infrastructure::TagArgumentUtils.resolve_value('some_literal_string', ctx)
  end

  def test_resolve_value_nil_or_empty_markup_input
    ctx = create_context
    assert_nil Jekyll::Infrastructure::TagArgumentUtils.resolve_value(nil, ctx)
