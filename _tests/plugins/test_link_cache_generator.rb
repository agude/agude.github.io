...
class TestLinkCacheGenerator < Minitest::Test
  def setup
    ...
    @other_page = create_doc(
      { 'title' => 'About Us', 'layout' => 'page' },
      '/about.html'
    )
    ...
    @site = create_site(
      ...
      [@author_page, @series_page, @other_page] # pages
    )
  end
  ...
  def test_generator_builds_cache_correctly
    ...
    assert_equal(
      { 'url' => '/authors/jane-doe.html', 'title' => 'Jane Doe' },
      author_cache['jane doe']
    )
    # Check pen name
    assert_equal(
      { 'url' => '/authors/jane-doe.html', 'title' => 'Jane Doe' },
    ...
    # Ensure other pages are not in author cache
    assert_nil author_cache['about us']

    # --- Assert Series Cache ---
    series_cache = cache['series']
    refute_nil series_cache
    assert_equal(
      { 'url' => '/series/foundation.html', 'title' => 'The Foundation' },
      series_cache['the foundation']
    )
    assert_nil series_cache['about us']

    # --- Assert Book Cache ---
    book_cache = cache['books']
    refute_nil book_cache
    assert_equal(
      { 'url' => '/books/book-one.html', 'title' => 'Book One' },
      book_cache['book one']
    )
    # Ensure unpublished and untitled books are not cached
    assert_nil book_cache['unpublished book']
    assert_empty book_cache.keys.select { |k| k.nil? || k.empty? }
  end
  ...
  def test_generator_handles_missing_books_collection
    ...
    assert_equal(
      { 'url' => '/authors/jane-doe.html', 'title' => 'Jane Doe' },
      site_no_books.data['link_cache']['authors']['jane doe']
    )
  end
end
