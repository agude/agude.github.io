...
class TestDisplayBooksByTitleAlphaGroupTag < Minitest::Test
  ...
  def setup
    ...
    @context = create_context({}, { site: @site, page: create_doc({ 'path' => 'current_title_page.md'}, '/current_title_page.html') })
  ...
  def test_renders_log_message_if_books_collection_missing
    ...
    context_no_books = create_context({}, { site: site_no_books, page: create_doc({ 'path' => 'current_title_page.md'}, '/current_title_page.html') })
    ...
    assert_match %r{<!-- \[ERROR\] BOOK_LIST_UTIL_FAILURE: Reason='Required &#39;books&#39; collection not found in site configuration\.'\s*filter_type='all_books_by_title_alpha_group'\s*SourcePage='current_title_page\.md' -->}, output
    refute_match %r{<h2 class="book-list-headline">}, output, "No H2 headings should be rendered if collection is missing"
  end
  ...
  def test_renders_log_message_if_no_published_books_found
    ...
    context_empty_books = create_context({}, { site: site_empty_books, page: create_doc({ 'path' => 'current_title_page.md'}, '/current_title_page.html') })
    ...
    assert_match %r{<!-- \[INFO\] ALL_BOOKS_BY_TITLE_ALPHA_GROUP_FAILURE: Reason='No published books found to group by title\.'\s*SourcePage='current_title_page\.md' -->}, output
    refute_match %r{<h2 class="book-list-headline">}, output, "No H2 headings should be rendered if no books are found"
  end
end
