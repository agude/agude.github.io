...
class TestRelatedBooksTag < Minitest::Test
  ...
  def test_logs_error_if_prerequisites_missing
    ...
    mock_logger_page_missing.expect(:error, nil) { |_p, m| m.include?('Missing prerequisites: page object') }
    ...
    mock_logger_coll_missing.expect(:error, nil) do |_p, m|
      m.include?('Missing prerequisites: site.collections[&#39;books&#39;]')
    end
  ...
  def test_series_current_book_number_unparseable_falls_back_to_all_series_sorted_numerically
    ...
    s1b1 = create_book_obj('S1B1', 'Series 1', 1, ['Auth'], 10, 's1b1', true, books_collection)
    s1b2 = create_book_obj('S1B2', 'Series 1', 2, ['Auth'], 9,  's1b2', true, books_collection)
    current_page_bad_num = create_book_obj('Current BadNum', 'Series 1', 'xyz', ['Auth'], 1, 'curr_bad_num', true,
                                           books_collection)
    ...
    assert_match %r{<!-- \[INFO\] RELATED_BOOKS_SERIES_FAILURE: Reason='Current page has unparseable book_number \(&#39;xyz&#39;\)\. Using all series books sorted by number\.' PageURL='/books/curr_bad_num\.html' Series='Series 1' SourcePage='books/curr_bad_num\.html' -->},
                 output
  end
  ...
  def test_series_provides_zero_books_author_and_recent_fill
    books_collection = MockCollection.new([], 'books')
    current_page = create_book_obj('Current In SeriesX', 'Series X', 1, ['Author X'], 0, 'curr_sx', true,
                                   books_collection)
    books_collection.docs = [current_page, @author_X_book1_old, @author_X_book2_recent, @recent_unrelated_book1,
  ...
  def test_series_provides_one_book_author_and_recent_fill_remaining_two
    ...
    author_Y_book_other_recent = create_book_obj('Other AY Book Recent', nil, nil, ['Author Y'], 3, 'ay_other_rec',
                                                 true, books_collection)
    books_collection.docs = [current_page, s1b2, author_Y_book_other_recent, @recent_unrelated_book1,
                             @recent_unrelated_book2].compact
    site = create_site(@site_config_base.dup, { 'books' => books_collection.docs })
    context = create_context({}, { site: site, page: current_page })

    output = render_tag(context)
    rendered_titles = extract_rendered_titles(output)
    expected_titles = [s1b2.data['title'], author_Y_book_other_recent.data['title'],
                       @recent_unrelated_book1.data['title']]
    assert_equal expected_titles, rendered_titles
  end
  ...
  def test_multi_author_current_page_uses_either_author_for_fallback
    books_collection = MockCollection.new([], 'books')
    current_page_multi = create_book_obj('Current Multi-Author', 'UniqueSeries', 1, ['Author A', 'Author B'], 0,
                                         'curr_multi', true, books_collection)
  ...
  def test_recent_fallback_when_series_and_author_provide_less_than_max
    books_collection = MockCollection.new([], 'books')
    current_page = create_book_obj('Current S1B1', 'Series Beta', 1, ['Author B'], 10, 'curr_s1b1', true,
                                   books_collection)
    books_collection.docs = [current_page, @recent_unrelated_book1, @recent_unrelated_book2, @recent_unrelated_book3,
    ...
    output = render_tag(context)
    rendered_titles = extract_rendered_titles(output)
    expected_titles = [@recent_unrelated_book1.data['title'], @recent_unrelated_book2.data['title'],
  ...
  def test_world_of_trouble_scenario_with_new_series_logic
    ...
    wot_book = create_book_obj('World of Trouble', tlp_series_name, 3, bhw_authors_array, 0, 'wot', true,
                               books_collection)
    cc_book  = create_book_obj('Countdown City', tlp_series_name, 2, bhw_authors_array, 300, 'cc', true,
                               books_collection)
    tlp_book = create_book_obj('The Last Policeman', tlp_series_name, 1, bhw_authors_array, 365, 'tlp', true,
                               books_collection)
    recent_unrelated_fill = create_book_obj('Recent Unrelated Fill', nil, nil, ['Another Author'], 5, 'ru_fill', true,
                                            books_collection)
  ...
  def test_returns_empty_string_if_no_valid_related_books
    books_collection = MockCollection.new([], 'books')
    current_page_alone = create_book_obj('Alone Again', nil, nil, ['Solo Author'], 0, 'alone_again', true,
                                         books_collection)
  ...
  def test_excludes_archived_reviews_from_recommendations
    ...
    related_canonical = create_book_obj('Related Canonical', 'Series A', 2, ['Auth'], 9, 'related_canon', true,
                                        books_collection)
    related_archived = create_book_obj('Related Archived', 'Series A', 3, ['Auth'], 8, 'related_archive', true,
                                       books_collection, { 'canonical_url' => '/some/path' })
  ...
  def test_includes_books_with_external_canonical_url
    books_collection = MockCollection.new([], 'books')
    current_page = create_book_obj('Current Book', 'Series A', 1, ['Auth'], 10, 'current', true, books_collection)
    related_external = create_book_obj('Related External', 'Series A', 2, ['Auth'], 5, 'related_ext', true,
                                       books_collection, { 'canonical_url' => 'http://some.other.site/path' })
