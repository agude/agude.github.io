...
class TestBookListUtilsAllBooksByYearDisplay < Minitest::Test # Renamed class
  ...
  def setup
    ...
    @book_2024_mar_setup = create_doc({ 'title' => 'Book Mar 2024 Setup', 'date' => Time.utc(2024, 3, 10), 'published' => true }, '/b2024mar_setup.html')
    @book_2023_jun_setup = create_doc({ 'title' => 'Book Jun 2023 Setup', 'date' => Time.utc(2023, 6, 20), 'published' => true }, '/b2023jun_setup.html')
    @unpublished_2023_setup = create_doc({ 'title' => 'Unpublished 2023 Book Setup', 'date' => Time.utc(2023, 7, 7), 'published' => false }, '/unpub2023_setup.html')
    @book_string_date_2022_setup = create_doc({ 'title' => 'Book String Date 2022 Setup', 'date' => "2022-11-11", 'published' => true }, '/b_str_date_2022_setup.html')
  ...
  def test_get_data_for_all_books_by_year_display_correct_grouping_and_sorting
    ...
    book_2024_mar = create_doc({ 'title' => 'Book Mar 2024', 'date' => Time.utc(2024, 3, 10), 'published' => true }, '/b2024b.html')
    book_2024_jan = create_doc({ 'title' => 'Book Jan 2024', 'date' => Time.utc(2024, 1, 15), 'published' => true }, '/b2024a.html')
    ...
    book_2023_dec = create_doc({ 'title' => 'Book Dec 2023', 'date' => Time.utc(2023, 12, 1), 'published' => true }, '/b2023a.html')
    book_2023_jun = create_doc({ 'title' => 'Book Jun 2023', 'date' => Time.utc(2023, 6, 20), 'published' => true }, '/b2023b.html')

    book_2022_oct = create_doc({ 'title' => 'Book Oct 2022', 'date' => Time.utc(2022, 10, 5), 'published' => true }, '/b2022oct.html')
    ...
    assert_equal ["2024", "2023", "2022"], actual_year_order, "Year groups not sorted correctly"
    ...
    assert_equal expected_2024_titles, group_2024[:books].map { |b| b.data['title'] }, "Books in 2024 group not sorted correctly by date"
    ...
    assert_equal expected_2023_titles, group_2023[:books].map { |b| b.data['title'] }, "Books in 2023 group not sorted correctly by date"
    ...
    assert_equal expected_2022_titles, group_2022[:books].map { |b| b.data['title'] }, "Books in 2022 group not sorted correctly by date"
  end
  ...
  def test_get_data_for_all_books_by_year_display_handles_books_with_non_time_dates_gracefully
    ...
    book_current_time_date = create_doc({ 'title' => 'Current Time Date Book', 'published' => true }, '/current_time_date.html')
    # Book with a string date that create_doc will parse
    book_specific_string_date = create_doc({ 'title' => 'Specific String Date Book', 'date' => "2022-07-01", 'published' => true }, '/specific_string_date.html')

    site_with_mixed_dates = create_site({}, { 'books' => [book_current_time_date, book_specific_string_date, @book_2023_jun_setup] })
  ...
  def test_get_data_for_all_books_by_year_display_no_books_logs_info
    ...
    assert_match %r{<!-- \[INFO\] ALL_BOOKS_BY_YEAR_DISPLAY_FAILURE: Reason='No published books found to group by year\.'\s*SourcePage='current_page\.html' -->}, data[:log_messages]
  end
