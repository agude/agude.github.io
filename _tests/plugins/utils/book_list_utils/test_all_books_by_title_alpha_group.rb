...
class TestBookListUtilsAllBooksByTitleAlphaGroup < Minitest::Test # Renamed class
  def setup
    ...
    @unpublished_book = create_doc({ 'title' => 'Unpublished Alpha Book', 'published' => false, 'date' => Time.now },
                                   '/unpub_alpha.html')
    @archived_book = create_doc({ 'title' => 'Archived Book', 'published' => true, 'canonical_url' => '/some/path' },
                                '/archived.html')
    @external_canonical_book = create_doc(
      { 'title' => 'External Canon Book', 'published' => true,
        'canonical_url' => 'http://othersite.com' }, '/external.html'
    )
  ...
  def test_get_data_for_all_books_by_title_alpha_group_correct_grouping_and_sorting
    ...
    expected_hash_titles = [@book_only_an.data['title'], @book_empty_title_sort.data['title'],
  ...
  def test_get_data_for_all_books_by_title_alpha_group_no_books_logs_info
    ...
    assert_match(/<!-- \[INFO\] ALL_BOOKS_BY_TITLE_ALPHA_GROUP_FAILURE: Reason='No published books found to group by title\.'\s*SourcePage='current_page\.html' -->/,
                 data[:log_messages])
  end
  ...
  def test_get_data_for_all_books_by_title_alpha_group_collection_missing_logs_error
    ...
    assert_empty data[:alpha_groups]
    assert_match(/<!-- \[ERROR\] BOOK_LIST_UTIL_FAILURE: Reason='Required &#39;books&#39; collection not found in site configuration\.'\s*filter_type='all_books_by_title_alpha_group'\s*SourcePage='current_page\.html' -->/,
                 data[:log_messages])
  end
end
