...
class TestBookListUtilsAllBooksByAuthorDisplay < Minitest::Test
  ...
  def setup
    ...
    @book_aa_s1_b1 = create_doc({ 'title' => 'AA: Series One, Book 1', 'series' => 'Series One', 'book_number' => 1, 'book_authors' => [@author_a_cap_name], 'published' => true }, '/aa_s1b1.html')
    @book_aa_s1_b0_5 = create_doc({ 'title' => 'AA: Series One, Book 0.5', 'series' => 'Series One', 'book_number' => 0.5, 'book_authors' => [@author_a_cap_name], 'published' => true }, '/aa_s1b0_5.html')
    @book_aa_standalone_zeta = create_doc({ 'title' => 'Zeta Standalone by AA', 'book_authors' => [@author_a_cap_name], 'published' => true }, '/aa_sa_zeta.html')
    @book_aa_standalone_apple = create_doc({ 'title' => 'Apple Standalone by AA', 'book_authors' => [@author_a_cap_name], 'published' => true }, '/aa_sa_apple.html')


    # By author beta
    @book_ab_standalone = create_doc({ 'title' => 'Standalone by ab', 'book_authors' => [@author_b_lower_name], 'published' => true }, '/ab_sa.html')

    # By Author Charlie (canonical and pen name)
    @book_ac_canonical = create_doc({ 'title' => 'Book by Author Charlie', 'book_authors' => [@author_c_canonical_name], 'published' => true }, '/ac_canon.html')
    @book_ac_pen_name = create_doc({ 'title' => 'Book by Charles Penotti', 'book_authors' => [@author_c_pen_name], 'published' => true }, '/ac_pen.html')

    # Co-authored book by Author Alpha and author beta
    @coauthored_aa_ab = create_doc({ 'title' => 'Co-authored AA & ab', 'book_authors' => [@author_a_cap_name, @author_b_lower_name], 'published' => true }, '/coauth_aa_ab.html')

    # Books with malformed or no valid author data
    @book_no_author = create_doc({ 'title' => 'No Author Book', 'book_authors' => [], 'published' => true }, '/no_auth.html')
    @book_nil_author = create_doc({ 'title' => 'Nil Author Book', 'book_authors' => nil, 'published' => true }, '/nil_auth.html')
    @book_empty_author = create_doc({ 'title' => 'Empty Author Book', 'book_authors' => [' '], 'published' => true }, '/empty_auth.html')
    @book_array_empty_author = create_doc({ 'title' => 'Array Empty Author Book', 'book_authors' => ["", "  "], 'published' => true }, '/arr_empty_auth.html')
    @unpublished_book = create_doc({ 'title' => 'Unpublished Book by AA', 'book_authors' => [@author_a_cap_name], 'published' => false }, '/unpub_aa.html')
  ...
  def test_get_data_for_all_books_by_author_display_correct_structure_and_sorting
    ...
    assert_equal expected_author_names_ordered, actual_author_names_ordered, "Authors not sorted correctly by canonical name"
    ...
    assert_equal expected_standalone_titles, author_a_standalone_titles, "Standalone books for Author Alpha are not sorted alphabetically"
    ...
    assert_equal @book_aa_s1_b0_5.data['title'], series_one_group[:books][0].data['title'], "Series One books not sorted correctly by book_number"
    assert_equal @book_aa_s1_b1.data['title'], series_one_group[:books][1].data['title'], "Series One books not sorted correctly by book_number"
