...
class TestBookListUtilsAllBooksByAuthorDisplay < Minitest::Test # Renamed class
  ...
  def setup
    ...
    @author_c_cap_name = "Author Charlie" # Third author for ordering

    # Books by Author Alpha
    @book_aa_s1_b1 = create_doc({ 'title' => 'AA: Series One, Book 1', 'series' => 'Series One', 'book_number' => 1, 'book_author' => @author_a_cap_name, 'published' => true, 'date' => Time.now }, '/aa_s1b1.html')
    @book_aa_s1_b0_5 = create_doc({ 'title' => 'AA: Series One, Book 0.5', 'series' => 'Series One', 'book_number' => 0.5, 'book_author' => @author_a_cap_name, 'published' => true, 'date' => Time.now }, '/aa_s1b0_5.html')
    @book_aa_standalone_zeta = create_doc({ 'title' => 'Zeta Standalone by AA', 'book_author' => @author_a_cap_name, 'published' => true, 'date' => Time.now }, '/aa_sa_zeta.html')
    @book_aa_standalone_apple = create_doc({ 'title' => 'Apple Standalone by AA', 'book_author' => @author_a_cap_name, 'published' => true, 'date' => Time.now }, '/aa_sa_apple.html')

    # Books by author beta
    @book_ab_s2_b1 = create_doc({ 'title' => 'ab: Series Two, Book 1', 'series' => 'Series Two', 'book_number' => 1, 'book_author' => @author_b_lower_name, 'published' => true, 'date' => Time.now }, '/ab_s2b1.html')
    ...
    @book_ac_s3_b1 = create_doc({ 'title' => 'AC: Series Three, Book 1', 'series' => 'Series Three', 'book_number' => 1, 'book_author' => @author_c_cap_name, 'published' => true, 'date' => Time.now }, '/ac_s3b1.html')
    @book_ac_standalone = create_doc({ 'title' => 'Standalone by AC', 'book_author' => @author_c_cap_name, 'published' => true, 'date' => Time.now }, '/ac_sa.html')


    # Books with no author or problematic author data
    @book_nil_author = create_doc({ 'title' => 'Nil Author Book', 'book_author' => nil, 'published' => true, 'date' => Time.now }, '/nil_auth.html')
    @book_empty_author = create_doc({ 'title' => 'Empty Author Book', 'book_author' => ' ', 'published' => true, 'date' => Time.now }, '/empty_auth.html')
    @unpublished_book = create_doc({ 'title' => 'Unpublished Book by AA', 'book_author' => @author_a_cap_name, 'published' => false, 'date' => Time.now }, '/unpub_aa.html')
    ...
    @books_for_by_author_tests = [
      ...
      @book_nil_author, @book_empty_author, @unpublished_book
    ]
  ...
  def test_get_data_for_all_books_by_author_display_correct_structure_and_sorting
    ...
    author_a_data = data[:authors_data][0] # First due to sort
    assert_equal @author_a_cap_name, author_a_data[:author_name]
    assert_equal 2, author_a_data[:standalone_books].size
    assert_equal @book_aa_standalone_apple.data['title'], author_a_data[:standalone_books][0].data['title'] # Apple before Zeta
    assert_equal @book_aa_standalone_zeta.data['title'], author_a_data[:standalone_books][1].data['title']
    ...
    author_b_data = data[:authors_data][1] # Second due to sort
    assert_equal @author_b_lower_name, author_b_data[:author_name]
    assert_equal 1, author_b_data[:standalone_books].size
    assert_equal @book_ab_standalone.data['title'], author_b_data[:standalone_books][0].data['title']
    ...
    # --- Assert Author Charlie's Data ---
    author_c_data = data[:authors_data][2] # Third due to sort
  ...
  def test_get_data_for_all_books_by_author_display_excludes_nil_empty_and_unpublished_authors
    data = get_all_books_by_author_data
    all_book_titles_rendered = data[:authors_data].flat_map do |author_data|
      (author_data[:standalone_books] + author_data[:series_groups].flat_map { |sg| sg[:books] })
        .map { |b| b.data['title'] }
    end

    refute_includes all_book_titles_rendered, @book_nil_author.data['title']
    refute_includes all_book_titles_rendered, @book_empty_author.data['title']
    refute_includes all_book_titles_rendered, @unpublished_book.data['title']
  end

  def test_get_data_for_all_books_by_author_display_logs_if_no_valid_author_books
    site_no_valid_authors = create_site({}, { 'books' => [@book_nil_author, @book_empty_author] })
