...
class TestBookCardUtils < Minitest::Test
  def setup
    ...
    @context = create_context({},
                              { site: @site, page: create_doc({ 'path' => 'current_card_test.md' }, '/current.html') })
  ...
  def test_render_book_card_no_authors_logs_warning
    ...
    assert_match(/<!-- \[WARN\] BOOK_CARD_MISSING_AUTHORS_FAILURE: Reason='&#39;book_authors&#39; field resolved to an empty list\.'\s*book_title='Anonymous Tales'\s*SourcePage='current_card_test\.md' -->/,
                 final_output)
  end
