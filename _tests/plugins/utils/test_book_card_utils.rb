...
class TestBookCardUtils < Minitest::Test
  ...
  def test_render_book_card_success
    mock_base_data = {
      ...
      raw_title: "My Book Title",
      log_output: ""
    }
    mock_prepared_title = "My Prepared Book Title"
    mock_description_html = "<p>Book excerpt.</p>"

    # This is what AuthorLinkUtils.render_author_link is expected to return
    mock_author_link_content_html = "<a href=\"/authors/test-author\"><span class=\"author-name\">Test Author</span></a>"
    # This is what RatingUtils.render_rating_stars is expected to return
    mock_rating_stars_content_html = "<div class=\"book-rating star-rating-4\">****</div>" # No extra newline in mock

    # These are the complete strings BookCardUtils will put into the extra_elements_html array
    ...
    expected_card_data_to_renderer = {
      ...
      image_alt: "Book cover of My Book Title.",
  ...
  def test_render_book_card_minimal_data
    book_minimal = create_doc({ 'title' => 'Minimal Book' }, '/minimal.html') # No author, rating, image, excerpt
    mock_base_data = {
      site: @site, data: book_minimal.data, absolute_url: "/minimal.html",
      absolute_image_url: nil, raw_title: "Minimal Book", log_output: ""
    }
    mock_prepared_title = "Minimal Book Prepared"
    mock_description_html = "" # Extractor returns "" if no excerpt

    captured_card_data = nil
    CardDataExtractorUtils.stub :extract_base_data, mock_base_data do
      LiquidUtils.stub :_prepare_display_title, mock_prepared_title do
        CardDataExtractorUtils.stub :extract_description_html, mock_description_html do
          # No need to stub AuthorLinkUtils or RatingUtils as they won't be called
