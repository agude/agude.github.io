...
class TestFrontPageFeedTag < Minitest::Test
  def setup
    ...
    @post1 = create_doc({ 'title' => 'Recent Post', 'date' => Time.parse("2024-05-28 10:00:00 UTC") }, '/post1.html', 'content', nil, MockCollection.new(nil, 'posts'))
    @book1 = create_doc({ 'title' => 'Recent Book', 'date' => Time.parse("2024-05-27 10:00:00 UTC") }, '/book1.html', 'content', nil, MockCollection.new(nil, 'books'))
    @post2 = create_doc({ 'title' => 'Older Post', 'date' => Time.parse("2024-05-26 10:00:00 UTC") }, '/post2.html', 'content', nil, MockCollection.new(nil, 'posts'))
    @book2 = create_doc({ 'title' => 'Older Book', 'date' => Time.parse("2024-05-20 10:00:00 UTC") }, '/book2.html', 'content', nil, MockCollection.new(nil, 'books'))
    @post3 = create_doc({ 'title' => 'Very Old Post', 'date' => Time.parse("2024-05-15 10:00:00 UTC") }, '/post3.html', 'content', nil, MockCollection.new(nil, 'posts'))
    @book3_oldest = create_doc({ 'title' => 'Very Old Book', 'date' => Time.parse("2024-05-10 10:00:00 UTC") }, '/book3.html', 'content', nil, MockCollection.new(nil, 'books'))
  ...
  def render_tag(markup = "", context = @context, feed_util_return_data = @mock_feed_items_default_limit)
    ...
    FeedUtils.stub :get_combined_feed_items, ->(args_hash) {
  ...
  def test_renders_feed_with_default_limit
    ...
    assert_match %r{<!-- ArticleCard for: Recent Post -->}, output
    assert_match %r{<!-- BookCard for: Recent Book -->}, output
    assert_match %r{<!-- ArticleCard for: Older Post -->}, output
    assert_match %r{<!-- BookCard for: Older Book -->}, output
    assert_match %r{<!-- ArticleCard for: Very Old Post -->}, output
    refute_match %r{Very Old Book}, output # Should be excluded by default limit of 5
  ...
  def test_renders_feed_with_specified_limit_literal
    ...
    assert_match %r{<!-- BookCard for: Recent Book -->}, output
    assert_match %r{<!-- ArticleCard for: Older Post -->}, output
    refute_match %r{Older Book}, output
    refute_match %r{Very Old Post}, output
  ...
  def test_handles_empty_feed_items_with_log
    ...
    assert_match %r{<!-- \[INFO\] FRONT_PAGE_FEED_FAILURE: Reason='No items found for the front page feed\.'\s*limit='#{Jekyll::FrontPageFeedTag::DEFAULT_LIMIT}'.*SourcePage='current_feed_page\.md' -->}, output
    refute_match %r{<div class="card-grid">}, output
  end

  def test_handles_unrecognized_item_type_with_log
    unknown_item = create_doc({ 'title' => 'Unknown Item', 'date' => Time.now }, '/unknown.html', 'content', nil, MockCollection.new(nil, 'unknown_collection_label'))
    output = render_tag("", @context, [unknown_item]) # FeedUtils returns this one unknown item

    assert_match %r{<div class="card-grid">}, output # Grid is still created
    assert_match %r{<!-- \[WARN\] FRONT_PAGE_FEED_FAILURE: Reason='Unknown item type in feed\.'\s*item_title='Unknown Item'\s*item_url='\/unknown\.html'\s*item_collection='unknown_collection_label'\s*SourcePage='current_feed_page\.md' -->}, output
    refute_match %r{<!-- ArticleCard for: Unknown Item -->}, output
    refute_match %r{<!-- BookCard for: Unknown Item -->}, output
