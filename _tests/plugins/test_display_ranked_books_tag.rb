...
class TestDisplayRankedBooksTag < Minitest::Test
  ...
  def test_dev_mode_raises_error_for_invalid_rating_in_list
    ...
    assert_match "Title 'Book Invalid Rating' (position 2 in 'page.ranked_list') has invalid non-integer rating: '\"five_stars\"'", err.message
  end
  ...
  def test_prod_mode_logs_error_to_console_for_non_existent_title_and_skips
    ...
    refute_match %r{<!--.*?DISPLAY_RANKED_BOOKS_FAILURE.*?-->}, output, "HTML comment should NOT be present in production"
